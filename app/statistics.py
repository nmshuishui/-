...
def get_qqcloud_cdn_stat(start_date, end_date, type):
    ...
    config = {
        'Region': 'gz',
        'secretId': 'AKIDOi1OCZSlajOO69DnVjeN3knARIOc65v2',
        'secretKey': 'DP1wZQENXHYs1oKc9CK9OskCV2odC60u',
        'method': 'post',
    }
    params = {
        ...
        'endDate': end_date,
        'projects.0': '0',
        'hosts.0': '0.fps.qq.kukuplay.com',
        'hosts.1': '1.fps.qq.kukuplay.com',
        'hosts.2': '2.fps.qq.kukuplay.com',
        'hosts.3': '3.fps.qq.kukuplay.com',
        'hosts.4': '4.fps.qq.kukuplay.com',
        'hosts.5': '5.fps.qq.kukuplay.com',
        'hosts.6': '6.fps.qq.kukuplay.com',
        'hosts.7': '7.fps.qq.kukuplay.com',
        'hosts.8': '8.fps.qq.kukuplay.com',
        'hosts.9': '9.fps.qq.kukuplay.com',
        'hosts.10': 'fps.qq.kukuplay.com',
        'statType': type,
        #     'flux'               #按流量查询
        #     'bandwidth',         #按带宽查询
        #     'requests',          #按请求数查询
    ...
    try:
        ...
        list = tmp.get('data').get('url_data')
    except Exception, e:
        list = ['exception:' ,e]
    ...
    if type == 'flux':
        ...
        for x in list:
            nums = x.get('value')
            if len(str(nums)) > 12:
                x['value'] = str(round(nums * 1.0 / (1000 * 1000 * 1000 * 1000), 2)) + 'TB'
            elif len(str(nums)) > 9:
                x['value'] = str(round(nums * 1.0 / (1000 * 1000 * 1000), 2)) + 'GB'
            elif len(str(nums)) > 6:
                x['value'] = str(round(nums * 1.0 / (1000 * 1000), 2)) + 'MB'
            elif len(str(nums)) > 3:
                x['value'] = str(round(nums * 1.0 / 1000, 2)) + 'KB'
            else:
                x['value'] = str(nums) + 'Byte'
    elif type == 'bandwidth':
        unit = '带宽使用量'
        for x in list:
            nums = x.get('value')
            if len(str(nums)) > 12:
                x['value'] = str(round(nums * 1.0 / (1000 * 1000 * 1000 * 1000), 2)) + 'Tbps'
            elif len(str(nums)) > 9:
                x['value'] = str(round(nums * 1.0 / (1000 * 1000 * 1000), 2)) + 'Gbps'
            elif len(str(nums)) > 6:
                x['value'] = str(round(nums * 1.0 / (1000 * 1000), 2)) + 'Mbps'
            elif len(str(nums)) > 3:
                x['value'] = str(round(nums * 1.0 / 1000, 2)) + 'Kbps'
            else:
                x['value'] = str(nums) + 'byte'
    else:
        unit = '请求次数(次)'
...
#调用cdn请求的页面
@app.route('/data_cdn_stat/',methods=['GET', 'POST'])
@login_required
def data_cdn_stat():
    params = request.args if request.method == 'GET' else request.form
    day = datetime.date.today() + datetime.timedelta(days=0)
    start_date = params.get('start_date',day)
